...
2VARIABLE 2operator \ переменная содержащая адрес отложенного оператора и признак(флаг) 
: operator 2operator CELL+ ; \ выдать адрес ячейки с adr
\ adr f 2operator 2! запоминает адрес и флаг
\ operator->adr, то есть  operator @ дает adr, а 0 operator ! стирает addr, оставляет f
\ 2operator 2@ дает -- adr f  
\ f=0 - оператор имеет опкод
\ f=1 - оператор фальшивый, без опкода
...
\ текущий, но с id операнда равного строке adr u
    2>V 2operator 2@ 0= AND \ V: adr u 
    IF  operator @
    ...
    ELSE \ 0 или псевдокоманда, искать нет смысла
        V>DROP 0 \ нечего искать - выход с 0
    THEN      
    ;
