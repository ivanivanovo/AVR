...
: WriteChip ( adr u adr1  -- ) \ записать u байт с адреса adr в adr1 Memory
    ...
    IF  3DUP Memo! \ записать
        TwP PAUSE
        Verify \ проверить
        0= IF ." Верификация не удалась." CR TRUE THROW THEN
...
: CheckPreProg ( adr u adr1  -- ) \ 
    ...
    2DUP + EndMemory 1+ > ABORT" Дамп слишком велик для данного чипа." 
    AsWord? 
    IF \ проверка чётности, если запись идёт по словам
        2DUP 1 AND ABORT" Адрес записи должен быть чётным."
        1 AND ABORT" Количество записываемых байт должено быть чётным."
    THEN
    ;
...
: TheProg ( --)
    ['] Flash! CATCH 
    DUP -2 = IF ." Ошибка программатора -2!!! Лишний хаб? "  CR ABORT THEN
    IF 
        EraseChip  
        Flash! 
    THEN
