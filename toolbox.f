...
: >STACK ( x*i x*u u name -- x*i ) \ перместить u элементов в стек "name"
...
: HEX> ( "str" -- n) \ взят число как шестнадцатеричное
    HEX[ asNum ]HEX ; IMMEDIATE 
: 0x [COMPILE] HEX> ; IMMEDIATE 
: 0h [COMPILE] HEX> ; IMMEDIATE  
: BIN> ( "str" -- n) \ взят число как двоичное
    BIN[ asNum ]BIN ; IMMEDIATE 
: OCT> ( "str" -- n) \ взят число как восмеричное
    OCT[ asNum ]OCT ; IMMEDIATE 
: DEC> ( "str" -- n) \ взят число как десятичное
...
\ при каждом вызове этого слова выдается новое значение на 1больше предыдущего
