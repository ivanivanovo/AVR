...
: operand! ( j*x operator op# -- i*x) 
    ...
    ;
: comby ( j*x -- i*x)
    operator @  \ взять из operator адрес статьи ПРЕдыдущего оператора
    \ если не 0, то 
    IF  2operator 2@ \ op f
        IF   \ псевдокоманда!
           0 0 2operator 2! \ взял-выкинул ДО выполнения (защита от рекурсии)
           EXECUTE
        ELSE \ команда ассемблера
            \ выполнить его слова-обработчики операндов,
            >R 
            R@ 2 operand! R@ 1 operand!  
            \ получить клише,
            R> clishe @
            ...
            0 0 2operator 2! \ взял-выкинул ПОСЛЕ выполнения (защита от рекурсии)
        THEN
    THEN
    ;
:NONAME ( j*x operator f -- i*x) \ на стеке лежат операнды!!!
     ...
     2>R 
        ...
        WHILE    
            comby
        REPEAT 
     2R> 2operator 2! \ текущую команду отложить для последующей обработки
     ; IS coder  
