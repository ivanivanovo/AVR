...
<BITS  PREVIOUS \ слова в битовый словарь, но без поиска в нём
    : } ( bit bit ...bit  -- u) \  собрать биты по именам в одну маску
        ...
        \ изврат конечно, но позволяющий одинаково именовать и биты и регистры  
        OVER    
        BIT[ label>name ]BIT ?DUP \ по значению бита найти его имя,
        IF RAM[ str>label  ?DUP   \ а по имени найти одноимённый регистр
           IF label-value @ -ROT NIP THEN \ и взять его значение....
           ]RAM 
        ELSE DROP THEN 
        ;
BITS> 
